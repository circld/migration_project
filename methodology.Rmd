...
The second ratio is a test to see if the proposal distribution is symmetric. The proposal distribution is based on the distribution space defined above. By picking a random source and random destination, we are limiting ourselves to the number of states acheivable by moving one person from one location to another. Given $n$ locations, and $n_{p \neq 0}$ locations with non-zero populations, and the current popuation distribution state $x_t$, the proposal distribution $Q(x' | x_t)$ includes these $n n_{p \neq 0}$ states weighted equally, with zero probability of reaching any other state.

In most cases, our proposal distribution is symmetric. If we're considering a move between locations $i$ and $j$, representing a change in state from $x_t$ to $x'$, and locations $i$ and $j$ are both non-zero, the number of states accessible from either $x_t$ or $x'$ is simply $n n_{p \neq 0}$. The probability $Q(x_t | x') = Q(x' | x_t) = \frac{1}{n n_{p \neq 0}}$.
...
$$ a = \frac{u_i (p)}{u_j (p) - c_{ij}} \frac{1/(n n_{p \neq 0, x_t})}{1/(n n_{p \neq 0, x'})} $$
